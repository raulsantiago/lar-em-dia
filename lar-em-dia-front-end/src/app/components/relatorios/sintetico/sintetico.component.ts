...
import { ListaLucroHoraDTO } from 'src/app/dto/pedido/listaLucroHoraDTO';
...
export class SinteticoComponent implements OnInit {
  ...
  dateValue: Date;
  dateValueInicial: string;
  dateValueFinal: string;
  ...
  listaLucroHoraDTO: ListaLucroHoraDTO[];  
  quantitadeServicos: number = 0;
  lucroTotal:         number = 0;
  valorPorHora:       number = 0;
  fmtValorPorHora:    string;
  horaTotal:          number = 0;
  horaFracao:         number = 0;

  listaLucroValorHoraDTO: ListaLucroHoraDTO[];  
  quantitadeServicos2: number = 0;
  lucroTotal2:         number = 0;
  valorPorHora2:       number = 0;
  fmtValorPorHora2:    string;
  horaTotal2:          number = 0;
  horaFracao2:         number = 0;

  dateValue2:        Date;
  dateValueInicial2: string;
  dateValueFinal2:   string;
  exibe2: boolean = false;
  ...
  ngOnInit(): void {
    let loginPromide:  Promise<any> = this.loginClienteService.quantidadeClientesAtivos().toPromise();
    let pedidoPromise: Promise<any> = this.pedidoService.listaTodosPedidosLucroHora().toPromise();
    Promise.all([loginPromide, pedidoPromise]).then( (retorno ) => {
        this.quantidadeClientesAtivos = retorno[0];
        this.listaLucroHoraDTO        = retorno[1];

        if(this.listaLucroHoraDTO){        
            // quatidade total
            this.quantitadeServicos = this.listaLucroHoraDTO.length;
    
            // lucro total
            for (let index = 0; index < this.listaLucroHoraDTO.length; index++) {                
                this.lucroTotal = this.lucroTotal + (this.listaLucroHoraDTO[index].precoContratado - this.listaLucroHoraDTO[index].despesas);
            }
    
            // total hora
            for (let i = 0; i < this.listaLucroHoraDTO.length; i++) {
                let inicio = new Date(this.listaLucroHoraDTO[i].dataInicio);
                let fim    = new Date(this.listaLucroHoraDTO[i].dataFim);                
                this.horaTotal = this.horaTotal + (fim.getTime() - inicio.getTime());
            }
    
            // lucro por hora trabalhada
            this.horaFracao = this.horaTotal / 3600000;
            this.valorPorHora = this.lucroTotal / this.horaFracao;
            this.fmtValorPorHora = this.numberToReal(this.valorPorHora);
          }
  ...
  fecharValorHoraTrabalhada(){
    this.exibe2 = false;
    this.dateValue2 = null;
  }

  calcularValorHoraTrabalhada(){
      var date = new Date(this.dateValue2);
      var primeiroDia = new Date(date.getFullYear(), date.getMonth(), 1);
      var ultimoDia = new Date(date.getFullYear(), date.getMonth() + 1, 0);      
      this.dateValueInicial2 = this.datepipe.transform(primeiroDia, 'yyyy-MM-ddT00:00:00') == undefined || this.datepipe.transform(primeiroDia, 'yyyy-MM-ddT00:00:00') == null ? '' : this.datepipe.transform(primeiroDia, 'yyyy-MM-ddT00:00:00');
      this.dateValueFinal2 = this.datepipe.transform(ultimoDia, 'yyyy-MM-ddT23:59:59') == undefined || this.datepipe.transform(ultimoDia, 'yyyy-MM-ddT23:59:59') == null ? '' : this.datepipe.transform(ultimoDia, 'yyyy-MM-ddT23:59:59');
      this.pedidoService.listaPedidosLucroHora(this.dateValueInicial2, this.dateValueFinal2).subscribe( dado => {
          this.listaLucroValorHoraDTO = dado;
          this.calcularVrHrTrabalhada(this.listaLucroValorHoraDTO);
      });

  }

  calcularVrHrTrabalhada(lista: ListaLucroHoraDTO[]){
    this.lucroTotal2 = 0;    
    this.horaTotal2 = 0; 
    this.horaFracao2 = 0;
    this.valorPorHora2 = 0;
    this.fmtValorPorHora2 = '';
      if(lista){
          this.exibe2 = true;        
        // quatidade total
        this.quantitadeServicos2 = lista.length;

        // lucro total
        for (let index = 0; index < lista.length; index++) {                
            this.lucroTotal2 = this.lucroTotal2 + (lista[index].precoContratado - lista[index].despesas);
        }

        // total hora
        for (let i = 0; i < lista.length; i++) {
            let inicio = new Date(lista[i].dataInicio);
            let fim    = new Date(lista[i].dataFim);                
            this.horaTotal2 = this.horaTotal2 + (fim.getTime() - inicio.getTime());
        }

        // lucro por hora trabalhada
        this.horaFracao2 = this.horaTotal2 / 3600000;
        this.valorPorHora2 = this.lucroTotal2 / this.horaFracao;
        this.fmtValorPorHora2 = this.numberToReal(this.valorPorHora2);
      }
  ...
  grafico(){
    this.listaGraficoDTO = null;
    this.dados = null;
    var date = new Date(this.dateValue);
    var primeiroDia = new Date(date.getFullYear(), date.getMonth(), 1);
    var ultimoDia = new Date(date.getFullYear(), date.getMonth() + 1, 0);    
    this.dateValueInicial = this.datepipe.transform(primeiroDia, 'yyyy-MM-ddT00:00:00') == undefined || this.datepipe.transform(primeiroDia, 'yyyy-MM-ddT00:00:00') == null ? '' : this.datepipe.transform(primeiroDia, 'yyyy-MM-ddT00:00:00');
    this.dateValueFinal = this.datepipe.transform(ultimoDia, 'yyyy-MM-ddT23:59:59') == undefined || this.datepipe.transform(ultimoDia, 'yyyy-MM-ddT23:59:59') == null ? '' : this.datepipe.transform(ultimoDia, 'yyyy-MM-ddT23:59:59');    
    let pedidosGraficoPromise: Promise<any> = this.pedidoService.listaPedidosGrafico(this.dateValueInicial, this.dateValueFinal).toPromise();
    Promise.all([pedidosGraficoPromise]).then( (retorno ) => {
      this.listaGraficoDTO = [];
      this.dados = [];
      this.lucroServicosTotal = 0;
      this.fmtLucroServicosTotal = '';
      this.options = '';
      this.graficoData = '';
      this.dia1 = [];
        this.dia2= [];
      ...
      this.fmtLucroServicosTotal = this.numberToReal(this.lucroServicosTotal);
            ...
            this.options = {
                title: {
                    display: true,
                    text: 'Lucro e Quantidade de Serviços por Dia',
                    fontSize: 20
                },
                tooltips: {
                    mode: 'index',
